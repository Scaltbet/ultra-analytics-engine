score = c.get('score', {}).get('value')
                    ...
                    dados_validos = True
                    if str(c.get('team', {}).get('id')) == str(id_time):
                        gols_pro = int(score)
                    else:
            ...
            return {
                ...
                "porcentagem_ambas_marcam": round((ambas_marcam_count / jogos_processados) * 100, 1),
                "porcentagem_over_1_5": round((over_1_5_count / jogos_processados) * 100, 1),
                "porcentagem_over_2_5": round((over_2_5_count / jogos_processados) * 100, 1)
            }
        except Exception as e:
            print(f"[SCALT HISTORICO] Erro ao processar time {id_time}: {str(e)}")
            return None